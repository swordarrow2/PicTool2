...
public class DownloadRunnable implements Runnable {
    ...
    public void run() {
        ...
        if (pictureInfoJavaBean.isAnimPicture) {
            ...
            try {
                ...
                String path = FileTool.getSaveFileAbsPath(zipName, FileType.pixivAnimate, FileFormat.FileType.gif_89a);
			...
			if (SharedPreferenceHelper.getBoolean("deleteZipAfterMakeGif")) {
			    file.delete();
            }
        } else {
    ...
    private void downloadEnd() {
        mengProgressBar.context.runOnUiThread(new Runnable() {
				@Override
				public void run() {
					String[] downloadedFilesName = FileTool.getDefaultFolder(FileType.pixivZIP).list();
