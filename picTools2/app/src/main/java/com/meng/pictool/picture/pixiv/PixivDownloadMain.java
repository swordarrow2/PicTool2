package com.meng.pictool.picture.pixiv;
...
import com.meng.pictool.database.*;
...
public class PixivDownloadMain extends BaseFragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        database = DataBaseHelper.getInstance(PixivDataBase.class);
    ...
    private void init(View view) {
        ...
        tabHost.setup();
        tabHost.addTab(tabHost.newTabSpec("zero").setIndicator("发发发").setContent(R.id.pixiv_download_main_browser));
        ...
        String[] filesName = FileTool.getAppFile(FunctionSavePath.pixivZIP, "").list();
                ...
                new AlertDialog.Builder(getActivity())
                        ...
                        .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface p1, int p2) {
                                ...
                                try {
                                    FileTool.savePreDownload(gson.toJson(likeJavaBean));
                                } catch (IOException e) {
        ...
        if (preDownloadJson.exists()) {
            try {
                likeJavaBean = gson.fromJson(FileTool.readString(preDownloadJson), LikeJavaBean.class);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
    ...
    private void checkFailed() {
        final ArrayList<String> failedURLs = database.getAllFailed();
    ...
    OnClickListener onClickListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            int id = v.getId();
            if (id == R.id.fab_add_mine) {
                File jsonFile = new File(FileTool.getPreDownloadJsonPath());
                if (jsonFile.exists()) {
                    try {
                        likeJavaBean = gson.fromJson(FileTool.readString(jsonFile), LikeJavaBean.class);
                    } catch (Exception e) {
                        throw new RuntimeException(e);
                    }
                } else {
                ...
                menuStar.close(true);
            } else if (id == R.id.fab_start_download) {
                startDownload();
            }
        }
    };
